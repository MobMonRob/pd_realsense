...
roslib.load_manifest('object_detection_rs')


class Detector:

    def __init__(self):
	# load Ros Topics to subscribe to
	self.settings = JsonManager("../parameters/settings.json").load_json()
	self.settingsCamera = JsonManager("../parameters/settingsCamera.json").load_json()
	# Subscribe to Ros Topics
        rospy.Subscriber(self.settingsCamera["DepthImageTopic"], Image, self.callback)
	rospy.Subscriber(self.settingsCamera["PointCloudTopic"], PointCloud2, self.save_point_cloud)
	...
	# adjust image processing parameters to unit of publishing ('m', 'cm', 'mm')
	camera_min = self.settings["camera_min"]*self.settingsCamera["unit"]
	camera_max = self.settings["camera_max"]*self.settingsCamera["unit"]
	camera_thresh = self.settings["camera_thresh"]*self.settingsCamera["unit"]
        # Delete areas further away than ...
        thresh = (camera_thresh - camera_min) * 255 / \
                 (camera_max - camera_min)

	# all image values larger than thresh are set to 0, all image values smaller than thresh are not altered
    ...
    def convert_img_msg(self, img_msg):	
        ...
        float_image = self.cv_bridge.imgmsg_to_cv2(img_msg, "passthrough")
	# adjust image processing parameters to unit of publishing ('m', 'cm', 'mm')
	camera_min = self.settings["camera_min"]*self.settingsCamera["unit"]
	camera_max = self.settings["camera_max"]*self.settingsCamera["unit"]
	camera_thresh = self.settings["camera_thresh"]*self.settingsCamera["unit"]
        ...
        # Adjust Image
	# make sure that float_image can be modified
	float_image.setflags(write = 1)
        #float_image[float_image < 10] = self.settings["camera_thresh"]


        float_image = (float_image - camera_min) / \
                      (camera_max - camera_min)
        float_image[float_image > (camera_max*2)] = 1
        float_image[float_image < 0] = 0

        # Convert 32fc1 to 8uc1
	# u1 = unsigned integer of length 1 Byte
	...
	#while len(points_3d) < 1:
        while len(points_3d) < 1 | i < 10:
	     ...
	     # append Point only if z is in meaningful range
	     #if 0.27 < z < 0.36:
	     if  self.settings["camera_min"] < z < self.settings["camera_max"]:
